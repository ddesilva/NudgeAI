...
enum Exporter {
    ...
    static var legacySessionsRoot: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("CueSessions", isDirectory: true)
    }
    ...
    static func export(annotations: [Annotation], date: Date = Date()) throws -> Result {
        ...
        let folder = sessionsRoot.appendingPathComponent("Nudge-\(stamp)", isDirectory: true)
        try fm.createDirectory(at: folder, withIntermediateDirectories: true)

        var mdLines: [String] = []
        var promptLines: [String] = []
        mdLines.append("# Nudge AI session — \(displayStamp(date))")
        mdLines.append("")
    ...
    private static func writePNG(_ image: NSImage, to url: URL) throws {
        guard let tiff = image.tiffRepresentation,
            ...
            throw NSError(domain: "NudgeAI", code: 1, userInfo: [NSLocalizedDescriptionKey: "PNG encode failed"])
        }
    ...
    private static func writeJSON(annotations: [Annotation], folder: URL, date: Date) throws {
        ...
        try data.write(to: folder.appendingPathComponent("nudge.json"))
    }
